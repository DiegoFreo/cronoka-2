...
// 👇 Importando o modal reutilizável e atualizado
import { ModalCadastroPiloto } from '@/app/components/ModalCadastroPiloto'; 
...
export default function CronometragemPage() {
  ...
  // Controle de Abertura do Modal Global
  ...
  // 🔥 REGRA DE NEGÓCIO: Ordenação oficial do grid de corrida
  const ordenarPilotosPorRanking = (lista: PilotoCorrida[]): PilotoCorrida[] => {
    return [...lista].sort((a, b) => {
      // 1ª Regra: Quem tem mais voltas completadas fica na frente
      if (b.voltas.length !== a.voltas.length) {
        return b.voltas.length - a.voltas.length;
      }
      // 2ª Regra (Desempate): Se estão na mesma volta, quem cruzou o pórtico primeiro fica na frente
      if (b.voltas.length > 0) {
        return a.tempoUltimaPassagem - b.tempoUltimaPassagem;
      }
      // Critério base: Ordena por número da moto se ninguém completou volta ainda
      ...
      try {
        ...
        if (res.ok && Array.isArray(dados)) {
          const pilotosFormatados: PilotoCorrida[] = dados.map((p: any) => ({
            id: p._id || p.id,
            numero: p.numero_piloto || p.numero,
            nome: p.nome,
            tagRfid: p.tagId || p.tagRfid,
            categoria: p.categoriaTexto || p.categoriaNome || 'Geral',
            voltas: [],
    ...
    setPilotos((prevPilotos) => {
      ...
      const gridAtualizado = prevPilotos.map((piloto) => {
        ...
        if (piloto.tagRfid) {
          ...
          tagRfidBate = 
            tagBancoCompleta === termoBusca || 
            tresUltimosBanco === termoBusca || 
            tresUltimosBancoLimpo === termoBuscaLimpo;
        }
  ...
  const excluirPrimeiraVolta = (pilotoId: string) => {
    ...
    setPilotos(prev => {
      const novaLista = prev.map(p => p.id === pilotoId ? { ...p, voltas: p.voltas.slice(1) } : p);
      return ordenarPilotosPorRanking(novaLista);
    });
  };
  ...
  const lidarComPilotoAdicionadoProModal = (pilotoInjetado: any) => {
    ...
    const pilotoFormatado: PilotoCorrida = {
      id: pilotoInjetado.id || pilotoInjetado._id,
      numero: pilotoInjetado.numero_piloto || pilotoInjetado.numero,
      nome: pilotoInjetado.nome,
      tagRfid: pilotoInjetado.tagId || pilotoInjetado.tagRfid || '',
      categoria: pilotoInjetado.categoriaTexto || pilotoInjetado.categoriaNome || 'Alinhado na Pista',
      voltas: [],
      tempoUltimaPassagem: 0,
      statusPista: 'NORMAL'
    };

    setPilotos(prevGrid => ordenarPilotosPorRanking([...prevGrid, pilotoFormatado]));
  };
  ...
  return (
    <div className="space-y-6 font-sans bg-[#070707] min-h-screen text-white p-6">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-800 text-sm font-mono">
                {pilotos.map((piloto, index) => {
                  ...
                  const ehLider = index === 0 && totalVoltas > 0;
                        ...
                        <div className="flex flex-wrap gap-1.5 max-w-xl">
                          {piloto.voltas.map((tempo, idx) => (
                            <div key={idx} className="flex items-center gap-1 bg-black/60 border border-gray-800 px-2 py-1 rounded text-xs">
                              ...
                              <button 
                                type="button"
                                onClick={() => editingVolta ? salvarEdicaoVolta() : setEditingVolta({ pilotoId: piloto.id, index: idx, valor: (tempo / 1000).toFixed(3) })}
                                className="text-gray-600 hover:text-red-400 ml-1"
                              >
                                {editingVolta?.pilotoId === piloto.id && editingVolta?.index === idx ? <Check size={12} /> : <Edit2 size={10} />}
                              </button>
                            </div>
                          ))}
                        </div>
      ...
      <ModalCadastroPiloto
        ...
        onSuccess={lidarComPilotoAdicionadoProModal}
      />
    </div>
  );
}